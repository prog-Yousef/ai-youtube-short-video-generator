...
export const GenerateVideoData = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const RenderVideo = await step.run("renderVideo", async () => {
      ...
      // Optionally, handle non-success cases here.
    });

    /**
     * Using AWS Lambda (FAST RENDERING)
     */
    /*
    const RenderVideoUsingAWSLambda = await step.run("RenderVideoUsingAWSLambda", async () => {
      // Render Video using AWS Lambda
      ...
      const { renderId, bucketName } = await renderMediaOnLambda({
        region: "us-east-1",
        functionName: process.env.REMOTION_AWS_FUNCTION_NAME, // Or use above functionName
